...
SELECT r.room_id, r.room_type, sum((b.check_out - b.check_in) * r.price_per_day) from rooms r
JOIN bookings b ON r.room_id = b.room_id
GROUP BY r.room_id;
-- Hiển thị tổng doanh thu của từng loại phòng
select r.room_type, sum((b.check_out - b.check_in) * r.price_per_day) from rooms r
join bookings b on r.room_id = b.room_id
...
select r.room_type, count(b.booking_id) from rooms r
join bookings b on r.room_id = b.room_id
group by r.room_type
...
select r.room_id, r.room_type, count(b.booking_id) from rooms r
join bookings b on r.room_id = b.room_id
group by r.room_id
order by count(b.booking_id) desc limit 1;
