...
create view public_posts_feed as
select u.username as author, p.content, p.created_at, count(l.like_id) as like_count from posts p
join users u on p.user_id = u.user_id
join likes l on p.post_id = l.post_id
where p.privacy = 'public'
group by p.post_id, u.username, p.content, p.created_at;

create view public_posts_only as
select post_id, user_id, content, privacy, created_at from posts
where privacy = 'public'
with check option;

insert into public_posts_only (user_id, content, privacy) 
values (1, 'new public post', 'public');

insert into public_posts_only (user_id, content, privacy) 
values (2, 'private post via view', 'private');
...
-- câu 6:
-- không nên tạo index khi bảng nhỏ hoặc cột ít tính phân biệt
-- không nên index cột content vì dữ liệu dài, tốn bộ nhớ, ít hiệu quả
-- index làm chậm insert/update vì phải cập nhật thêm cấu trúc index
